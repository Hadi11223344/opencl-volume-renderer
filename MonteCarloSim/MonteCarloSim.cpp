...
void init()
{
  // Minimal OpenCL infrastructure
	...
	// simulation buffer
	simulationBufferGPU = clCreateBuffer(context, CL_MEM_READ_WRITE, 
		sizeof(float)*3*resolution*resolution*resolution, 
		NULL, NULL);
...
void resetSimulation(int resolution, cl_mem simulationBufferGPU)
{
	...
	size_t workSize[4] = {resolution, resolution, resolution, 3};
	clEnqueueNDRangeKernel(queue, resetSimulationKernel,
						  4, NULL, workSize, NULL, 0, NULL, NULL);
